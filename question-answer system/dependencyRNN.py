...
class DependencyRNN:
    ...
    def __init__(self, d, V, r, answer_idxs, embeddings=None, seed=0):
        ...
        def normalized_tanh(x):
            '''
            z = T.scalar()
            normed_t =  T.tanh(x) / T.sqrt(
                theano.reduce(
                    fn=(lambda item, prior: prior + item * item),
                    sequences=[x],
                    outputs_info=T.zeros_like(z) )[0]  )
            '''
            normed_t = T.tanh(x) / T.sqrt(T.square(x).sum())
            return normed_t
        ...
        def recurrence(n, hidden_states, hidden_sums, cost, x, r, p, wrong_ans, corr_ans):
            ...
            (in that order)
            '''
            hidden_states = T.set_subtensor( hidden_states[n] , self.f( T.dot(self.Wv, x[n] ) + self.b + hidden_sums[n]  )  )
            hidden_sums = T.set_subtensor (hidden_sums[p[n]], hidden_sums[p[n] ] + T.dot(r[n], hidden_states[n]))
            result, update = theano.scan(
                fn=( lambda wrong_sample, corr_ans, hidden_states, n: T.maximum(0, 1-T.dot(corr_ans, hidden_states[n]) + T.dot(wrong_sample, hidden_states[n] )  ) ),
                     sequences=[wrong_ans],
                     non_sequences=[corr_ans, hidden_states, n],
                     outputs_info=None
                )
            result_cost = T.sum(result)
            return hidden_states, hidden_sums, result_cost
